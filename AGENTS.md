...
- Порядок переключения с туннельной схемы: остановить туннель здесь (`pkill -f "192[.]168[.]10[.]160"` — именно regex-форма, иначе `pkill -f` убьёт собственный шелл, в чьей командной строке есть тот же текст; плюс проверить осиротевший `ssh -R`) → поднять API на ВМ → проверить `curl localhost:8000/api/v1/status` на ВМ → вопрос через домен. nginx при переключении менять не надо (`/api/` уже смотрит на `127.0.0.1:8000`).
